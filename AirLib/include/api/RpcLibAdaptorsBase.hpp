...
namespace airlib_rpclib
{
    ...
    public:
        ...
        {
            DetectionInfo info;
            std::unordered_map<std::string, Vector2r> bones;

            MSGPACK_DEFINE_MAP(info, bones);

            SkeletalDetectionInfo()
            {
            }
            
            SkeletalDetectionInfo(const msr::airlib::SkeletalDetectionInfo& d)
            {
                info = DetectionInfo(d.info);
                for (const auto& value: d.bones)
                {
                    bones.emplace(value);
                }
            }
            
            msr::airlib::SkeletalDetectionInfo to() const
            {
                msr::airlib::SkeletalDetectionInfo d;
                d.info = info.to();

                for (auto it = bones.cbegin(); it != bones.cend(); ++it) {
                    const std::string& key = it->first;
                    const Vector2r& value = it->second;

                    d.bones.insert({key, value.to()});
                }
                
                return d;
            }
            
            static std::vector<SkeletalDetectionInfo> from(
              const std::vector<msr::airlib::SkeletalDetectionInfo>& request)
            {
              std::vector<SkeletalDetectionInfo> request_adaptor;
              for (const auto& item : request)
                request_adaptor.push_back(SkeletalDetectionInfo(item));
            
              return request_adaptor;
            }
            static std::vector<msr::airlib::SkeletalDetectionInfo> to(
              const std::vector<SkeletalDetectionInfo>& request_adapter)
            {
              std::vector<msr::airlib::SkeletalDetectionInfo> request;
              for (const auto& item : request_adapter)
                request.push_back(item.to());
...
MSGPACK_ADD_ENUM(msr::airlib::WorldSimApiBase::TypedAsset);
